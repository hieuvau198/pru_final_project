...
public class CharacterStats : MonoBehaviour
{
    ...
    // In CharacterStats.cs, add the following method:
    public void Heal(int amount)
    {
        currentHealth = Mathf.Min(currentHealth + amount, GetMaxHealthValue());
        onHealthChanged?.Invoke();

        // Trigger a popup notification when healing occurs.
        if (NotificationManager.Instance != null)
        {
            NotificationManager.Instance.ShowNotification("Healed +" + amount + " HP", 2f);
        }
    }
